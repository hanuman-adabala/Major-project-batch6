...
seed(1337)
tensorflow.random.set_seed(x)tensorflow.random.set_seed(x)
...
learning_rate = 5e-5
epochs = 2
checkpoint = ModelCheckpoint("vgg16_classifier.h5", monitor = 'val_acc', verbose=1, save_best_only=True, save_weights_only=False, mode='auto', period=1)
...
plot_confusion_matrix(cm, classes = category_names, title='Confusion Matrix', normalize=False, figname = 'VGG16_Confusion_matrix_concrete.jpg')
...
learning_rate = 5e-5
epochs = 2
checkpoint = ModelCheckpoint("vgg16_classifier_augm.h5", monitor='val_acc', verbose=1, save_best_only=True, save_weights_only=False, mode='auto', period=1)
model.compile(loss="categorical_crossentropy", optimizer=optimizers.Adam(lr=learning_rate, clipnorm=1.), metrics = ['acc'])
history = model.fit_generator(train_generator, 
                              epochs=epochs, 
                              shuffle=True, 
...
plot_confusion_matrix(cm_aug, classes = category_names, title='Confusion Matrix', normalize=False, figname = 'Vgg16Confusion_matrix_Augm.jpg')
